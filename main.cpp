...
int main(int argc, char* argv[]) {
    ...
    app.add_option("constant,-c,--constant", constant_name, "Name of the mathematical constant to calculate")
        ->required()
        ->check(CLI::IsMember({"pi", "e", "phi", "gamma", "sqrt2", "sqrt3", "sqrt5", "apery", "catalan", "ln2", "ln10"}));
    
    app.add_option("precision,-p,--precision", precision, "Number of decimal places to calculate")
        ->required()
    ...
    std::cout << "Calculating " << constant_name << " to " << precision << " decimal places..." << std::endl;
    ...
    if (calculator != calculators.end()) {
        ...
        std::cerr << "Error: Unknown constant '" << constant_name << "'" << std::endl;
        return 1;
    }
